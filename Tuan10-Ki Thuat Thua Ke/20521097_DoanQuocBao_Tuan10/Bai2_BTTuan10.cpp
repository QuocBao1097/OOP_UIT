...
public:
    ...
    int GetSL();
    void nhap();
    SINHVIEN*nhap1();// them doi tuong moi
    void xuat();
...
{
    int DTBMax=-1, vt=-1;// khong co SVCQ
    // tim kiem SVCQ Dau tien
    for(int i=0; i<sl; i++)
            if(dtsv[i]->Getloai()==1)
            {
                vt=i;// ghi nhận vi tri dau tien
                DTBMax=dtsv[i]->GetDTB();// ghi nhận giá trị đầu tiên
                break;
            }
    if(vt!=-1)//nếu có SVCQ đầu tiên
    {
        for(int i=vt+1; i<sl; i++)
            if(dtsv[i]->Getloai()==1 && dtsv[i]->GetDTB()>DTBMax)
                DTBMax=dtsv[i]->GetDTB();
        //liệt kê SVCQ có ĐTB cao nhất
        for(int i=0; i<sl; i++)
            if(dtsv[i]->Getloai()==1 && dtsv[i]->GetDTB()==DTBMax)
                dtsv[i]->xuat();
    }
